class CryptosController < ApplicationController
  ...
  def index
    ...
    @my_cryptos = JSON.parse(@response)
  end
  ...
  def show
    ...
    @uri = URI(@url)
    @response = Net::HTTP.get(@uri)
    @show_cryptos = JSON.parse(@response)
  end
  ...
  private
    ...
    def crypto_params
      params.require(:crypto).permit(:symbol, :user_id, :amount_owned, :paid_per)
    end
    
    def correct_user
       @correct = current_user.cryptos.find_by(id: params[:id])
       redirect_to cryptos_path, notice: "Not Autorized to edit this entry" if @correct.nil?
    end
end
